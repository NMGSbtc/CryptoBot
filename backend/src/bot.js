...
bot.dialog("/profile", [
  ...
  function (session, results) {
    ...
    if (results.response.match(/login/gi)) {
      var url = 'https://www.coinbase.com/oauth/authorize?response_type=code&client_id=' + process.env.COINBASE_CLIENT_ID + '&redirect_uri=' + redirectURL + '&scope=wallet:accounts:read'
...
bot.dialog("/login", function (session) {
  console.log(encodeURIComponent(process.env.COINBASE_REDIRECT_URL));
  var url = 'https://www.coinbase.com/oauth/authorize?response_type=code&client_id=' + process.env.COINBASE_CLIENT_ID + '&redirect_uri=' + encodeURIComponent(process.env.COINBASE_REDIRECT_URL) + '&scope=wallet:accounts:read'
